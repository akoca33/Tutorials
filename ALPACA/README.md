...
* __Advanced Settings Options__ 
  ...
  * __Acceleration__: The user can check this box to use the optional accelerated deformable registration algorithm available [here](https://github.com/ohirose/bcpd). The user must then provide the path to the `win` folder containing the `bcpd.exe` file. Acceleration will then be applied, cutting runtime by a factor of ~5.
